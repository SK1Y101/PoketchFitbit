...
function Vorpal() {
  ...
  this._version = ''; // Program title

  this._title = ''; // Program description

  this._description = ''; // Program baner

  this._banner = ''; // Command line history instance

  this.cmdHistory = new this.CmdHistoryExtension(); // Registered `vorpal.command` commands and
  // their options.
  ...
  this._hooked = false;
  this._useDeprecatedAutocompletion = false; // Expose common utilities, like padding.

  this.util = VorpalUtil;
  this.Session = Session; // Active vorpal server session.
...
Vorpal.prototype.parse = function (argv, options) {
  ...
  var args = argv;
  var result = this;
  var catchExists = !(_.find(this.commands, {
    _catch: true
  }) === undefined);
  args.shift();
  args.shift();
...
vorpal.mode = function (name, desc, opts) {
  return this.command(name, desc, _.extend(opts || {}, {
    mode: true
  }));
};
/**
...
vorpal.prompt = function () {
  ...
  var userCallback = arguments.length > 1 ? arguments[1] : undefined;
...
vorpal._prompt = function (data) {
  ...
  var ssn = self.getSessionById(data.sessionId); // If we somehow got to _prompt and aren't the
...
vorpal._exec = function (item) {
  ...
  if (match) {
    ...
    item.args = self.util.buildCommandArgs(matchArgs, match, item, self.isCommandArgKeyPairNormalized); // If we get a string back, it's a validation error.
    ...
    if (match._mode === true && !item.session._mode) {
      ...
      item.session._mode = item.command; // Execute the mode's `init` function
...
vorpal._commandHelp = function (command) {
  ...
  var groups = _.uniq(matches.filter(function (cmd) {
    ...
    var prefix = "    ".concat(VorpalUtil.pad(cmd + ' *', width), "  ").concat(counts[cmd], " sub-command").concat(counts[cmd] === 1 ? '' : 's', ".");
    return prefix;
  });
...
vorpal.getSessionById = function (id) {
  ...
  var ssn = _.find(this.server.sessions, {
    id: id
  });
