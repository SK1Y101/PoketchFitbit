...
var util = {
  ...
  parseCommand: function parseCommand(command, commands) {
    ...
    function parsePipes() {
      ...
      var naivePipes = String(command).trim().split('|'); // Contruct empty array to place correctly split commands into.

      var newPipes = []; // We will look for pipe characters within these quotes to rejoin together.

      var quoteChars = ['"', '\'', '`']; // This will expand to contain one boolean key for each type of quote.
      ...
      var quoteTracker = {}; // The current command piece before being rejoined with it's over half.
      // Since it's not common for pipes to occur in commands, this is usually the entire command pipe.

      var commandPart = ''; // Loop through each naive pipe.
      ...
      for (var key in naivePipes) {
        ...
        commandPart += possiblePipe; // Loop through each individual character in the possible pipe tracking the opening and closing of quotes.

        for (var i = 0; i < possiblePipe.length; i++) {
          var _char = possiblePipe[i];

          if (quoteChars.indexOf(_char) !== -1) {
            quoteTracker[_char] = !quoteTracker[_char];
          }
  ...
  buildCommandArgs: function buildCommandArgs(passedArgs, cmd, execCommand, isCommandArgKeyPairNormalized) {
    ...
    var types = cmd._types || {}; // Make a list of all boolean options
    ...
    types["boolean"] = booleans.map(function (str) {
      ...
      var match = false;
      var strings = ["-".concat(str), "--".concat(str), "--no-".concat(str)];
    ...
    for (var m = 0; m < cmd.options.length; ++m) {
      ...
      var _short = String(o["short"] || '').replace(/-/g, '');

      var _long = String(o["long"] || '').replace(/--no-/g, '').replace(/^-*/g, '');

      var exist = parsedArgs[_short] !== undefined ? parsedArgs[_short] : undefined;
      exist = exist === undefined && parsedArgs[_long] !== undefined ? parsedArgs[_long] : exist;
      var existsNotSet = exist === true || exist === false;

      if (existsNotSet && o.required !== 0) {
        return "\n  Missing required value for option ".concat(o["long"] || o["short"], ". Showing Help:");
      }

      if (exist !== undefined) {
        args.options[_long || _short] = exist;
      }
    ...
    var _loop = function _loop(key) {
      ...
      if (optionFound === undefined) {
        if (cmd._allowUnknownOptions) {
          ...
          return {
            v: "\n  Invalid option: '".concat(opt, "'. Showing Help:")
          };
        }
      }
    };
  ...
  humanReadableArgName: function humanReadableArgName(arg) {
    ...
    return arg.required ? "<".concat(nameOutput, ">") : "[".concat(nameOutput, "]");
  },
