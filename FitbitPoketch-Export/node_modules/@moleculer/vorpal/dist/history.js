...
var os = require('os'); // Number of command histories kept in persistent storage
...
var History = function History() {
  this._storageKey = undefined; // Prompt Command History
  // Histctr moves based on number of times 'up' (+= ctr)
  //  or 'down' (-= ctr) was pressed in traversing
  // command history.

  this._hist = [];
  this._histCtr = 0; // When in a 'mode', we reset the
...
History.prototype.getNextHistory = function () {
  this._histCtr--; // Return empty prompt if the we dont have any history to show
...
History.prototype.newCommand = function (cmd) {
  ...
  this._histCtr = 0; // Don't store command in history if it's a duplicate.
  ...
  this._hist.push(cmd); // Only persist history when not in mode
