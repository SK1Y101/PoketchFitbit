...
function Session(options) {
  ...
  this._modeDelimiter = undefined; // Keeps history of how many times in a row `tab` was
  // pressed on the keyboard.

  this._tabCtr = 0;
  this.cmdHistory = this.parent.cmdHistory; // Special command mode vorpal is in at the moment,
...
session.getAutocompleteDeprecated = function (str, cb) {
  cb = cb || function () {}; // Entire command string
  ...
  trimmed = cut; // Set "trimmed" to command string after pipe
  ...
  var command = match ? _.find(this.parent.commands, {
    _name: match
  }) : undefined;

  if (!command) {
    command = _.find(this.parent.commands, {
      _catch: true
    });
...
session.execCommandSet = function (wrapper, callback) {
  ...
  this._completedCommands = 0; // Create the command instance for the first
  ...
  this.cancelCommands = function () {
    ...
    callCancel(wrapper.commandInstance); // Check if there is a cancel method on the promise
    ...
    self.parent.emit('client_command_cancelled', {
      command: wrapper.command
    });
    cbk(wrapper);
  };

  this.on('vorpal_command_cancel', self.cancelCommands); // Gracefully handles all instances of the command completing.
  ...
  this._commandSetCallback = function () {
    ...
    if (self.isLocal() && err) {
      ...
      self.log(stack);
      self.parent.emit('client_command_error', {
        command: wrapper.command,
        error: err
      });
    } else if (self.isLocal()) {
      self.parent.emit('client_command_executed', {
        command: wrapper.command
      });
    }
