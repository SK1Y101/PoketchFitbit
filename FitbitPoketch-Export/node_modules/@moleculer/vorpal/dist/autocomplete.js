...
var autocomplete = {
  ...
  match: function match(str, arr, options) {
    ...
    var longestMatchLength = matches.reduce(function (previous, current) {
      ...
      return previous;
    }).length; // couldn't resolve any further, return all matches
...
function getMatch(ctx, data, options) {
  ...
  var prefix = new Array(len - trimmed.length + 1).join(' '); // If we get an autocomplete match on a command, finish it.
...
function getMatchObject(input, commands) {
  ...
  var matchObject = match ? _.find(this.parent.commands, {
    _name: String(match).trim()
  }) : undefined;
  ...
  if (!matchObject) {
    matchObject = _.find(this.parent.commands, {
      _catch: true
    });
...
function getMatchData(input, cb) {
  ...
  if (midOption === true && !cmd._allowUnknownOptions) {
    ...
    for (var i = 0; i < cmd.options.length; ++i) {
      var _long = cmd.options[i]["long"];
      var _short = cmd.options[i]["short"];
  ...
  if (afterOption === true) {
    ...
    var shortMatch = _.find(cmd.options, {
      "short": opt
    });

    var longMatch = _.find(cmd.options, {
      "long": opt
    });
