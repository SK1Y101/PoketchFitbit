"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.AppScreenshotStreamCaptureResult = exports.AppScreenshotStreamCaptureParams = exports.ScreenshotCapabilities = void 0;
const t = require("io-ts");
const BulkData_1 = require("./BulkData");
const Structures_1 = require("./Structures");
exports.ScreenshotCapabilities = t.intersection([
    t.partial({
        stream: t.boolean,
    }),
    t.interface({
        imageFormats: t.array(t.string),
    }),
], 'ScreenshotCapabilities');
exports.AppScreenshotStreamCaptureParams = t.interface({
    stream: BulkData_1.StreamToken,
    imageFormat: t.string,
}, 'AppScreenshotStreamCaptureParams');
exports.AppScreenshotStreamCaptureResult = t.partial({
    length: Structures_1.NonNegativeInteger,
}, 'AppScreenshotStreamCaptureResult');
//# sourceMappingURL=Screenshot.js.map