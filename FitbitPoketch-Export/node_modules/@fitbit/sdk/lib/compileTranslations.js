...
function compileTranslations(defaultLanguage) {
    ...
    const transform = new stream_1.Transform({
        ...
        transform(file, _, next) {
            ...
            const languageTag = path_1.basename(file.basename, '.po');
            if (!languageTag_1.validateLanguageTag(languageTag)) {
                next(new plugin_error_1.default(PLUGIN_NAME, `Translation file ${file.basename} has a bad name. Translation files must be named ${humanize_list_1.default(Object.keys(ProjectConfiguration_1.Locales).map((tag) => tag + '.po'), { conjunction: 'or' })}.`, { fileName: file.relative }));
                return;
            }
            if (file.isBuffer()) {
                try {
