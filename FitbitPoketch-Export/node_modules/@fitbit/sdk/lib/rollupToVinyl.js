...
function isEmittedAsset(chunkOrAsset) {
    return !!chunkOrAsset.isAsset;
}
function rollupToVinyl(inputOptions, outputOptions) {
    const stream = new stream_1.Readable({ objectMode: true });
    stream._read = () => { };
    function generatePath(fileName) {
        return outputOptions.dir
            ? path_1.posix.join(outputOptions.dir, fileName)
            : fileName;
    }
    function emitAsset({ fileName, source }) {
        stream.push(new vinyl_1.default({
            contents: source instanceof Uint8Array
                ? Buffer.from(source)
                : Buffer.from(source, 'utf8'),
            path: path_1.resolve(process.cwd(), generatePath(fileName)),
        }));
    }
    function emitChunk({ code, fileName, map, isEntry }) {
        ...
        stream.push(new vinyl_1.default({
            ...
            path: path_1.resolve(process.cwd(), chunkPath),
            sourceMap: map,
        }));
    }
    rollup
        ...
        return bundle.generate(outputOptions).then(({ output }) => {
            for (const chunkOrAsset of output) {
                if (isEmittedAsset(chunkOrAsset))
                    emitAsset(chunkOrAsset);
                else
                    emitChunk(chunkOrAsset);
            }
            stream.push(null);
        });
    })
